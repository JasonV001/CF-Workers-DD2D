...
async function API2ip(APIs) {
	...
	const timeout = setTimeout(() => controller.abort(), 2000); // 2秒后触发

	try {
		// 使用Promise.allSettled等待所有API请求完成，无论成功或失败
		const responses = await Promise.allSettled(APIs.map(item => {
			// 检查是否是域名（非URL格式）
			if (!/^https?:\/\//i.test(item)) {
				// 直接返回域名解析结果
				return updateIPArrays([item]).then(result => {
					return { isDomain: true, result };
				});
			}
		...
		for (const response of responses) {
			...
			if (response.status === 'fulfilled') {
				...
				// 处理域名解析结果
				if (value && value.isDomain) {
					const [ipv4List, ipv6List] = value.result;
					IP4 = IP4.concat(ipv4List);
					IP6 = IP6.concat(ipv6List);
					
					// 记录日志
					ipv4List.forEach(ip => log(`API获取 ${item} 的 A记录 ${ip}`));
					ipv6List.forEach(ip => log(`API获取 ${item} 的 AAAA记录 ${ip}`));
					continue;
				}
